...
- Add `Sentry.metrics.count`, `gauge`, and `distribution` with typed attributes, units, and `options.metrics.beforeSend` on Apple, Android, and JVM. Unsupported targets remain no-ops ([#572](https://github.com/getsentry/sentry-kotlin-multiplatform/pull/572)).
