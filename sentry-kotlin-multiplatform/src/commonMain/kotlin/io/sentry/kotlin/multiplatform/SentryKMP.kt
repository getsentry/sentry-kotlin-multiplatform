...
public object Sentry {
  ...
  @HiddenFromObjC
  @Deprecated("Use init(OptionsConfiguration) instead.", ReplaceWith("Sentry.init(configuration)"))
  public fun init(context: Context, configuration: OptionsConfiguration) {
    SentryBridge.init(context, configuration)
  }

  /**
   * Sentry initialization with an option configuration handler. This is a convenience init for
   * direct initialization on Apple platforms.
   *
   * @param configuration Options configuration handler.
   */
  ...
  /**
   * Starts a new transaction and returns a new `Span` representing it. The `Span` can be used to
   * record additional information about the transaction or add child spans.
   *
   * @param name The name of the transaction.
   * @param operation The operation of the transaction, typically the name of the function or
   *   endpoint.
   * @return A new `Span` representing the transaction.
   */
  public fun startTransaction(name: String, operation: String): ISpan {
   ...
   * @return A new `Span` representing the transaction.
   */
  public fun startTransaction(name: String, operation: String, bindToScope: Boolean): ISpan {
