...
public open class SentryOptions {
  ...
  /**
   * This function is called by TracesSampler to determine if transaction is sampled - meant to be
   * sent to Sentry.
   */
  ...
  /** Enables/Disables capturing screenshots before an error. Available on iOS and Android. */
  ...
  /**
   * Enables or disables the attach view hierarchy feature when an error happened. This is only
   * available on iOS and Android.
   */
  public var attachViewHierarchy: Boolean = false

  /**
   * Enables or disables the feature to automatically capture HTTP client errors. This is enabled by
   * default.
   *
   * Available on Apple.
   */
   ...
   *
   * By default, only HTTP client errors with a response code between 500 and 599 are captured as
   * errors.
   *
   * Available on Apple.
   */
   ...
   *
   * By default, HTTP client errors from every target (.* regular expression) are automatically
   * captured.
   *
   * Available on Apple.
   */
