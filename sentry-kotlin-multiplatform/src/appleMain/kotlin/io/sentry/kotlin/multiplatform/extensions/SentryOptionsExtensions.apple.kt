...
/**
 * Applies the given options to this CocoaSentryOptions. This avoids code duplication for init on
 * iOS.
 */
...
internal fun CocoaSentryOptions.applyCocoaBaseOptions(options: SentryOptions) {
  ...
  options.environment?.let { environment = it }
  ...
  options.sampleRate?.let { sampleRate = NSNumber(double = it) }
  options.tracesSampleRate?.let { tracesSampleRate = NSNumber(double = it) }
  beforeSend = { event ->
    ...
    val packages =
        options.sdk
            ?.packages
            ?.map { mapOf("name" to it.name, "version" to it.version) }
            ?.toMutableList()
            ?: mutableListOf()
  ...
  beforeBreadcrumb = { cocoaBreadcrumb ->
    if (options.beforeBreadcrumb == null) {
      ...
      cocoaBreadcrumb
          ?.toKmpBreadcrumb()
          ?.let { options.beforeBreadcrumb?.invoke(it) }
          ?.toCocoaBreadcrumb()
    }
  }
  ...
  tracesSampler = {
    it?.let { context ->
      ...
      sampleRate?.let { unwrappedSampleRate -> NSNumber(unwrappedSampleRate) }
    }
  }
  ...
  failedRequestStatusCodes =
      options.failedRequestStatusCodes.map {
        SentryHttpStatusCodeRange(min = it.min.convert(), max = it.max.convert())
      }
}
