...
public actual class SentryEvent actual constructor() : SentryBaseEvent() {
  ...
  public constructor(jvmSentryEvent: JvmSentryEvent) : this() {
    ...
    jvmSentryEvent.exceptions?.let {
      exceptions = it.map { it.toKmpSentryException() }.toMutableList()
    }
    jvmSentryEvent.breadcrumbs?.let {
      breadcrumbs = it.map { it.toKmpBreadcrumb() }.toMutableList()
    }
