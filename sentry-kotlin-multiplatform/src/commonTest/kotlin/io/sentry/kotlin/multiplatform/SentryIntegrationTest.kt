...
class SentryIntegrationTest : BaseSentryTest() {
  ...
  fun `scope is correct when modified with capturing an event`() {
    ...
    Sentry.captureException(RuntimeException("test")) { it.setTag(expectedKey, expectedValue) }
  ...
  fun `local scope overrides global scope when capturing an event and then capturing another event`() {
    ...
    Sentry.captureException(RuntimeException("test")) { it.setTag("testABC", "valueABC") }

    Sentry.captureException(RuntimeException("test2")) { it.setTag("testABC", "valueABC") }
  ...
  fun `global scope sets tag correctly`() {
    ...
    Sentry.configureScope { it.setTag("testABC", "valueABC") }
  ...
  fun `global scope sets level correctly`() {
    ...
    Sentry.configureScope { it.level = expectedLevel }
