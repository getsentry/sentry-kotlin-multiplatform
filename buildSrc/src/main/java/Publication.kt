...
fun DistributionContainer.configureForMultiplatform(project: Project) {
  ...
  this.getByName("main").contents {
    ...
    from("build${sep}kotlinToolingMetadata") {
      rename {
        it.replace(
            "kotlin-tooling-metadata.json", "${project.name}-$version-kotlin-tooling-metadata.json")
      }
    }
    from("build${sep}libs") {
      ...
      rename { it.replace("multiplatform-kotlin", "multiplatform").replace("-metadata", "") }
    }
  }
  ...
  this.maybeCreate("jvm").contents {
    from("build${sep}publications${sep}jvm") { renameModule(project.name, "jvm", version) }
  ...
  this.maybeCreate("iosx64").contents {
    from("build${sep}publications${sep}iosX64") { renameModule(project.name, "iosx64", version) }
    from("build${sep}libs$sep") {
  ...
  this.maybeCreate("tvosx64").contents {
    from("build${sep}publications${sep}tvosX64") { renameModule(project.name, "tvosx64", version) }
    from("build${sep}libs$sep") {
...
private fun CopySpec.fromKlib(projectName: String, target: String, version: String) {
  ...
  from("build${sep}libs${sep}${target}${sep}main") {
    rename { it.replaceRange(pos, pos, "-${target.toLowerCase()}-$version") }
  }
  from("build${sep}classes${sep}kotlin${sep}${target}${sep}main${sep}klib") {
    rename { "$projectName-${target.toLowerCase()}-$version.klib" }
  }
}
...
private fun CopySpec.renameModule(projectName: String, renameTo: String = "", version: String) {
  ...
  rename { it.replace("module.json", "$projectName$target-$version.module") }
}
